...
src/core/fixtures/corpus/  twenty-four real entries this bot wrote, used to measure retrieval rather than assume it
...
- **The shortlist size is measured and will need re-measuring.** Ten, because at twenty-four entries the right entry for one question ranks ninth, and a shortlist that loses the answer makes the judge's job impossible. A larger knowledge base may rank it lower still. `npm run judge-eval` and the evaluation tests are how to find out.
