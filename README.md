...
2. **The knowledge base is searched.** The exact wording of a question asked before always finds its entry, immediately and for nothing. Everything else is shortlisted by BM25 and decided by a model — word-counting never answers anything by itself. That is measured rather than cautious: thresholds that let it decide served nothing wrong on twelve entries and two wrong answers on twenty-four, because a score grows with the corpus and no fixed number survives it. A second opinion costs cents and a few seconds against the dollar and the minute a fresh answer costs. On a knowledge base of a handful of entries, word statistics say almost nothing, so nothing is written off on lexical evidence alone: the whole base goes for that second opinion instead.
...
It has been run against a codebase it was not written for — a published library, shipped as compiled JavaScript with no comments — and answered accurately, in product language, with no code references. Spot-checking the answers against that library's source confirmed details a guess would not produce.
...
src/core/fixtures/corpus/  thirty-one real entries this bot wrote, used to measure retrieval rather than assume it
...
- **Retrieval never decides, and every question but an exact repeat costs a second opinion.** A few seconds and a few cents each. That is the price of not serving confident wrong answers: measured on twenty-four entries, the judge answered ten of ten answerable questions correctly and none of the seven it should not, where deciding on word-counting alone got two wrong.
- **The shortlist scales with the knowledge base, and its cap will fail first.** Half the entries, never fewer than ten, never more than twenty. A fixed number does not survive growth: the hardest question in the labelled set ranked its answer ninth at twenty-four entries and tenth at thirty-one. Erring large is nearly free — the same three unrelated questions are shortlisted at five as at twenty — so the risk is the cap, not the fraction. Past the point where twenty candidates is too few, a badly-phrased question gets derived again rather than found: expensive, not wrong.
- **Merging catches only obvious duplicates now.** At twenty-four entries the least alike pair describing one behaviour scores 0.18, below the most alike pair describing two different ones at 0.32. The distributions overlap, so no threshold catches every duplicate without merging neighbours. The bar is set for safety, and the rest accumulate.
...
- **Reading an unfamiliar codebase costs more.** Measured against a third-party library — compiled JavaScript, no comments, never seen before — answers cost about $1.28 and took 80 to 96 seconds, against $0.60–$1.15 and 70 to 85 seconds on this project's own source. About half as much again. `DOCSEARCHER_MAX_USD` is the only bound.
