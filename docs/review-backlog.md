...
### Everything measured so far was measured on thirty-one entries about two subjects
...
Growing it again to thirty-one in iteration 28 moved one more: the shortlist size, which was fixed at ten and about to lose an answer. It now scales with the knowledge base.

The lesson is consistent across all three: a number measured on a small corpus does not survive a larger one, and the remaining risk is always the most recently fixed number. Here that is the cap of twenty on the shortlist. The corpus is still about this bot and subscription billing.
